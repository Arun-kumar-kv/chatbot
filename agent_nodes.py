...
_COMPLAINT_TEXT_QUERIES = [
    (
        ...
        """SELECT
                  ...
                  mi.COMPLAINT_DESCRIPTION AS COMPLAINT_TEXT
...
def db_rag_node(state: AgentState, db_manager, **_) -> AgentState:
    ...
    return {
        **state,
        "vector_results_text":  full_context,
        "vector_results":       sample_rows,
...
def vector_search_node(state: AgentState, vector_store, db_manager=None, **_) -> AgentState:
    ...
    # ── db_rag ALWAYS goes through db_rag_node (SQL fetch) ───────────────────
    # Never use FAISS for db_rag — FAISS is top_k limited and gives incomplete data.
    # Full SQL fetch → map-reduce is the only correct path for qualitative analysis.
    if is_rag:
        if db_manager:
            logger.info("[VectorSearch] db_rag strategy → routing to SQL-based db_rag_node")
            return db_rag_node(state, db_manager=db_manager)
        else:
            logger.warning("[VectorSearch] db_rag but no db_manager available")
            return {**state, "vector_results": [], "vector_results_text": ""}

    # ── FAISS unavailable for non-rag strategies ──────────────────────────────
    ...
    # ── Build RAG context from FAISS metadata ─────────────────────────────────
    if results:
        chunks = []
        for r in results:
            text = str(r.get("text") or r.get("complaint_description") or "").strip()
            if not text:
                text_fields = [
                    "complaint_description", "resolution_notes", "notes",
                    "business_type", "remarks", "description", "mgmt_comments",
                ]
                parts = [f"{f.replace('_',' ').title()}: {str(r.get(f,''))}"
                         for f in text_fields if r.get(f) and len(str(r.get(f))) > 5]
                text = "\n".join(parts)
            if not text:
                parts = [f"{k.replace('_',' ').title()}: {v}"
                         for k, v in r.items()
                         if not k.startswith("_") and k not in ("source","table")
                         and isinstance(v, str) and len(v) > 3]
                text = "\n".join(parts)
            if not text:
                continue

            score    = r.get("_score", 0)
            source   = r.get("source") or r.get("table", "")
            tenant   = r.get("tenant_name") or r.get("tenant") or ""
            prop     = r.get("property_name") or r.get("property") or ""
            inc_type = r.get("incident_type") or r.get("category") or ""
            inc_date = r.get("incident_date") or r.get("date") or ""
            status   = r.get("status") or ""
            ref      = r.get("incident_no") or r.get("contract_number") or ""

            header_parts = [f"[Source: {source}  Score: {score:.2f}]"]
            if ref:      header_parts.append(f"Ref: {ref}")
            if tenant:   header_parts.append(f"Tenant: {tenant}")
            if prop:     header_parts.append(f"Property: {prop}")
            if inc_type: header_parts.append(f"Type: {inc_type}")
            if inc_date: header_parts.append(f"Date: {str(inc_date)[:10]}")
            if status:   header_parts.append(f"Status: {status}")

            chunks.append("\n".join(header_parts) + f"\n{text[:1200]}")

        results_text = (
            f"=== {len(chunks)} RECORDS for: '{query}' ===\n\n"
            + ("\n\n" + "─"*50 + "\n\n").join(chunks)
        ) if chunks else "No relevant records found."
    else:
        results_text = "No relevant records found."

    logger.info("[VectorSearch] strategy=%s → %d FAISS results", strategy, len(results))
...
Extract ALL complaint themes from this batch. For each theme found:
- Category name
- Count in this batch  
- 1-2 example quotes from the complaints (keep them short, max 100 chars each)
- Tenant / Property names where notable

Be exhaustive — do not skip any complaint. Return a structured summary of themes found in this batch."""


def _format_records_for_llm(records: list, start: int = 0) -> str:
    """Format a list of complaint records into compact text for LLM input."""
    lines = []
    for i, r in enumerate(records, start + 1):
        parts = [f"#{i}"]
        if r.get("tenant"):        parts.append(r["tenant"])
        if r.get("property"):      parts.append(r["property"])
        if r.get("incident_type"): parts.append(r["incident_type"])
        if r.get("category"):      parts.append(r["category"])
        if r.get("date"):          parts.append(str(r["date"])[:10])
        if r.get("status"):        parts.append(r["status"])
        header = " | ".join(parts)
...
def _fast_rag_analysis(question: str, records: list, db_manager=None) -> str:
    """
    Fast single-call RAG analysis. Strategy:
      1. Pre-summarise records in Python (no LLM) → keyword frequency table
      2. Take a representative sample of 60 records (spread across dataset)
      3. Single LLM call with: counts + sample texts → full answer in ~5 seconds
    """
    total = len(records)
    if total == 0:
        return "No complaint records found in the database."

    logger.info("[FAST-RAG] Analysing %d records with single LLM call", total)

    # ── Step 1: Python-side keyword frequency (no LLM cost) ──────────────────
    import re as _re
    KEYWORDS = {
        "AC / Air Conditioning":  ["ac", "air condition", "cooling", "hvac", "chiller", "aircon"],
        "Water / Plumbing":       ["water", "leak", "plumb", "pipe", "flood", "drain", "tap", "faucet"],
        "Electricity":            ["electric", "power", "light", "wiring", "socket", "circuit", "switch"],
        "Elevator / Lift":        ["elevator", "lift"],
        "Noise":                  ["noise", "loud", "sound", "disturb"],
        "Pest / Insects":         ["pest", "insect", "ant", "cockroach", "rat", "mouse", "bug"],
        "Parking":                ["parking", "car park", "vehicle"],
        "Internet / WiFi":        ["internet", "wifi", "wi-fi", "network", "broadband"],
        "Cleanliness":            ["clean", "dirty", "trash", "garbage", "waste", "hygiene"],
        "Amenities":              ["pool", "gym", "amenity", "facilities", "beach", "garden"],
        "Door / Window":          ["door", "window", "lock", "key", "handle", "seal"],
        "Communication / Delay":  ["no response", "delay", "waiting", "ignored", "follow up", "respond"],
        "Repeated Issue":         ["again", "still", "repeated", "10th time", "multiple times", "not fixed"],
        "Urgent":                 ["urgent", "emergency", "asap", "immediately", "critical"],
    }

    counts = {cat: 0 for cat in KEYWORDS}
    for r in records:
        text_lower = str(r.get("text", "")).lower()
        for cat, kws in KEYWORDS.items():
            if any(kw in text_lower for kw in kws):
                counts[cat] += 1

    # Sort by frequency, keep non-zero
    sorted_counts = sorted(
        [(cat, cnt) for cat, cnt in counts.items() if cnt > 0],
        key=lambda x: x[1], reverse=True
    )

    # Build frequency table string
    freq_lines = [f"  {cat:<30} {cnt:>4} / {total}  ({cnt*100//total}%)"
                  for cat, cnt in sorted_counts]
    freq_table = "\n".join(freq_lines) if freq_lines else "  (no keyword matches found)"

    # ── Step 2: Representative sample — spread evenly across full dataset ─────
    SAMPLE_SIZE = 60
    if total <= SAMPLE_SIZE:
        sample = records
    else:
        # Take every N-th record so sample covers the full time range
        step   = total // SAMPLE_SIZE
        sample = records[::step][:SAMPLE_SIZE]

    # Format sample compactly — one line header + truncated text
    sample_lines = []
    ...
    prompt = (
        f"Question: {question}\n\n"
        f"DATABASE ANALYSIS — {total} total complaint records\n"
        f"{'='*55}\n\n"
        f"KEYWORD FREQUENCY SCAN (all {total} records):\n"
        f"{freq_table}\n\n"
        f"SAMPLE COMPLAINTS ({len(sample)} records, spread across full dataset):\n"
        f"{sample_text}\n\n"
        f"{'='*55}\n"
        f"Using the frequency table above (which covers ALL {total} records) "
        f"and the sample complaints for context, provide a complete analysis:\n"
        f"- List each issue category with its count and % (from the frequency table)\n"
        f"- Quote 1-2 actual examples from the sample for each major category\n"
        f"- Name specific tenants/properties where notable\n"
        f"- Rank from most to least frequent\n"
        f"- End with 2-3 prioritized recommendations"
    )

    try:
        resp = llm_invoke_with_retry(llm, [
            SystemMessage(content=_RAG_SYSTEM_PROMPT),
            HumanMessage(content=prompt),
        ], context="fast_rag")
        answer = resp.content.strip()
        logger.info("[FAST-RAG] Done: %d chars", len(answer))
        return answer
    except Exception as exc:
        logger.error("[FAST-RAG] LLM failed: %s", exc)
        # Return the frequency table as plain text fallback
        return (
            f"## Complaint Analysis — {total} Records\n\n"
            f"**Keyword Frequency Scan:**\n```\n{freq_table}\n```\n\n"
            f"*(LLM synthesis failed: {exc})*"
        )


def synthesise_answer_node(state: AgentState, db_manager=None, **_) -> AgentState:
    raw_q    = state["user_question"]
    user_q   = state.get("resolved_question") or raw_q
    ...
    vec_text = state.get("vector_results_text") or ""
    vec_records = state.get("vector_results") or []

    # ── RAG path ───────────────────────────────────────────────────────────────
    rag_has_content = (
        vec_text
        and "FAISS unavailable" not in vec_text
        and "No relevant records" not in vec_text
        and len(vec_text.strip()) > 80
    )

    if strategy in ("vector_only", "db_rag") and rag_has_content:

        # db_rag: vec_records is a list of dicts with keys: source, tenant, property, text, etc.
        # These come from db_rag_node (SQL fetch) — always use map-reduce for full coverage
        if strategy == "db_rag" and isinstance(vec_records, list) and len(vec_records) >= 0:
            sources = {}
            for r in vec_records:
                src = r.get("source", "Maintenance Incidents") if isinstance(r, dict) else "unknown"
                sources[src] = sources.get(src, 0) + 1
            logger.info("[Synthesise-RAG] db_rag: %d sample records, context %d chars",
                        len(vec_records), len(vec_text))

            llm = _get_llm(max_tokens=2000)
            prompt = (
                f"Question: {user_q}\n\n"
                f"{vec_text}\n\n"
                f"Using the category counts above (which reflect the full database) "
                f"and the sample complaints for examples, write a complete analysis."
            )
            try:
                resp   = llm_invoke_with_retry(llm, [
                    SystemMessage(content=_RAG_SYSTEM_PROMPT),
            ...
            llm = _get_llm(max_tokens=2000)
            num_results = len(vec_records)
            rag_user_msg = (
                f"Question: {user_q}\n\n"
                f"Analyzing {num_results} records:\n\n"
                f"{vec_text[:14000]}"
            )
            try:
                response = llm_invoke_with_retry(llm, [
                ...
                answer = f"Analysis failed due to API issue. Raw data:\n{vec_text[:3000]}"

        logger.info("[Synthesise-RAG] %d chars answer, %d records", len(answer), len(vec_records))
    ...
    # ── db_rag with no content ─────────────────────────────────────────────────
    if strategy == "db_rag" and not rag_has_content:
        llm = _get_llm(max_tokens=600)
        try:
            response = llm_invoke_with_retry(llm, [
                SystemMessage(content=_RAG_SYSTEM_PROMPT),
                HumanMessage(content=(
                    f"Question: {user_q}\n\n"
                    "No complaint records were found in the database. "
                    "Suggest the user ask 'How many complaints by type?' for SQL-based counts."
                )),
            ], context="rag_fallback")
            answer = response.content.strip()
        except Exception:
            answer = "No complaint records were found. Try asking: **'How many complaints by type?'**"
    ...
    llm = _get_llm()  # uses LLM_MAX_TOKENS from config via get_llm default
    zero_row_note = ""
